...
export default function AllResultsPage() {
  ...
  return <div className="result-shell"><Link className="back-link" href={`/result/${id}`}><ArrowLeft size={17} />おすすめ結果へ戻る</Link><header className="result-heading"><span className="eyebrow"><CalendarDays size={14} /> All candidates</span><h1>期間内の全候補カレンダー</h1><p>{schedule.title} · 選択期間の全日を月ごとに表示しています。日付を選ぶと、その日の全候補を確認できます。</p></header><ResultsCalendar
  dates={generateDates(schedule)}
  candidates={ranking}
