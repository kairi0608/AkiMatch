...
import { ArrowLeft, CalendarCheck2, Sparkles, UsersRound } from "lucide-react";
...
export default function ResultPage() {
  ...
  useEffect(() => {
    setSchedule(scheduleRepository.getSchedule(id));
  ...
  if (!schedule) return <div className="empty-state page-empty"><strong>結果が見つかりませんでした</strong><Link className="btn" href="/">ホームへ戻る</Link></div>;
  ...
  return <div className="result-shell">
    ...
    <section className="ranking-section"><div className="section-head"><div><span className="eyebrow">Ranking</span><h2>おすすめ日時</h2></div><small>候補を開くと参加状況を確認できます</small></div><ResultRanking candidates={ranking} participantCount={participants.length} /></section>
  </div>;
}
